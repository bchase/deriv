...
import nibble/lexer
import deriv/util
import deriv/types.{type File, File, type Output, Output, type Gen, Gen, type Derivation, Derivation, type DerivFieldOpt, DerivFieldOpt}
...
fn parse_types_and_derivations(file: File) -> List(#(CustomType, List(Derivation))) {
  ...
  |> list.map(type_with_derivations(_, file.src))
  |> result.values
}
